...
func (s *Service) addDailyEvents(c *gin.Context, userId uint) (int, interface{}) {
	temp := new(dailyInput)
	err := c.BindJSON(temp)
	if err != nil {
...
func (s *Service) deleteDailyEvents(c *gin.Context, userId uint) (int, interface{}) {
	id := c.Query("id")
	if id == "" {
	...
	tx := s.DB.Begin()
	if tx.Where("id = ? AND user_id = ?", id, userId).Delete(&dailyEvent{}).RowsAffected != 1 {
		tx.Rollback()
		return makeErrorReturn(500, 50000, "Can't Insert Into Database")
	}
	tx.Commit()
...
func (s *Service) modifyDailyEvents(c *gin.Context, userId uint) (int, interface{}) {
	id := c.Query("id")
	if id == "" {
		return makeErrorReturn(404, 40400, "Unable To Parse Parameters")
	}
	...
	tx := s.DB.Begin()
	if tx.Model(&dailyEvent{}).Where("id = ? AND user_id = ?", id, userId).Updates(&dailyEvent{
		Title: temp.Title,
		Extra: temp.Extra,
	}).RowsAffected != 1 {
		tx.Rollback()
